...
% Returns:%   corners: the locations of the four outer corners as a 4x2 array, in
...
%       is 0.5*max(H(:))%   NHoodSize:  Size of suppression neighborhood.  Default is
%       [size(H,1)/50, size(H,2)/50].  Must be odd numbers.
myThresh = ceil(0.05*max(H(:)));
NHoodSize = ceil([size(H,1)/50, size(H,2)/50]);
% Force odd size
if mod(NHoodSize(1),2)==0  
    NHoodSize(1) = NHoodSize(1)+1; 
end
if mod(NHoodSize(2),2)==0 
    NHoodSize(2) = NHoodSize(2)+1; 
end
peaks = houghpeaks(H, ...
...
% This function draws lines on whatever image is being displayed.% Input parameters:
